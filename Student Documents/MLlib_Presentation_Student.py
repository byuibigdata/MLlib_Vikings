...
# MAGIC %md
# MAGIC 
# MAGIC # Student Workthrough File
...
# MAGIC 
# MAGIC [This website](https://spark.apache.org/docs/latest/ml-features.html) shows a comprehensive list of transformers and estimators that are available to use in MLlib. 
# MAGIC 
...
final_df = output.select("--Fill in Here--")
#final_df.display()

# COMMAND ----------

# Here you decide how to split your data
train_data, test_data = final_df.randomSplit(["--Fill in Here--"])
...
lm = LinearRegression(labelCol = "--Fill in Here--")
...
iris_mllib = spark._______________(dat) # to make a spark dataframe
iris_mllib = (iris_mllib # this step is to rename the columns. Hint: its similar to withColumn
              ._________________('Sepal.Length', 'Sepal_Length')
              ._________________('Sepal.Width', 'Sepal_Width')
              ._________________('Petal.Length', 'Petal_Length')
              ._________________('Petal.Width', 'Petal_Width')
             )
...
iris_mllib = (
    _____________( # insert function imported above
        inputCol='Species',
        outputCol='Species2',
        handleInvalid='keep')
    .fit(__________) # insert dataset here from previous code chunk
    .transform(__________) # insert dataset here from previous code chunk
    .drop('Species')
...
assembler = _______________(
    inputCols=[____, ____, ____, ____], # what are our features? You can use dataset.dtypes to figure this out
    outputCol='features') # this argument names the array of our features
...
pyspark_rf = RandomForestClassifier(
    labelCol = _________, # this is how we set the target variable. Which column is our target?
    featuresCol = __________ # this is how we tell it which features to look at, remember, its an array of features.
) 

# COMMAND ----------

pyspark_rf_model = pyspark_rf.___(training_data) # training the model
predictions = pyspark_rf_model._________(test_data) # making predictions. This is equivalent to sklearn's predict's method
...
# MAGIC %md
# MAGIC ### Clustering Model Building
...
### Importing data
#R  data
women = sm.datasets.get_rdataset("_____", "datasets", cache=True).data
dat = pd.DataFrame(_____)
df1=spark.createDataFrame(_____)
...
feature_vector = VectorAssembler(inputCols = _____, outputCol = 'features') ### Note for self: maybe explore this functionality before presentation --Hathway will probably ask what it does specifically
#Adding our features vector column to our original dataset
data = feature_vector.transform(_____)
final_data = feature_vector.transform(_____).select('features')
...
''' 
For this step, we will use the pyspark StandardScalar function. 
...
'''
scaler = StandardScaler(inputCol = '_____', outputCol = 'zfeatures', withStd = True, withMean = False)
scalerModel = scaler.fit(_____)  ### Check more in detail what this is doing
cluster_input_data = scalerModel.transform(_____)
...
### Training the model
kmeans = KMeans(featuresCol = '_____', k=_____) #Where K is the number of clusters
model1 = kmeans.fit(_____)

predictions = model1.transform(_____)
...
cluster_output = model1.transform(_____).select('zfeatures', 'prediction')
rows = cluster_output.collect()
df_predictions = spark.createDataFrame(_____)
display(_____)
...
for k in range (2,16): #You can test as many k values as you would like however it gets significantly longer when doing so
    kmeans = KMeans(featuresCol = '_____', k=k)
    model2 = kmeans.fit(_____)
    predictions = model2.transform(_____)
    evaluator = ClusteringEvaluator(_____)
    silhouette = evaluator.evaluate(_____)
...
graph_df = (cluster_output.withColumn("_____", vector_to_array("zfeatures"))).select(['features'] + ['prediction'])
graph_df=graph_df.withColumn('_____', F.col('features').getItem(0))
graph_df=graph_df.withColumn('_____', F.col('features').getItem(1))   
...
sns.scatterplot(data=pd_graph, x="_____", y="_____", hue = "prediction",palette="colorblind")

# COMMAND ----------

# MAGIC %md
# MAGIC ### Pipeline Building
...
# split the data
training_data_a, test_data_a = iris_mllib.randomSplit([___,___])
# make sure your target column is called label
strings_indexed = StringIndexer(inputCol=___,outputCol=___,handleInvalid='keep')
# breaking up what the features and targets are
feat_cols = ['Sepal_Length', 'Sepal_Width', 'Petal_Length', 'Petal_Width']
assembler = VectorAssembler(inputCols=feat_cols, outputCol='features')
# put the model here 
pyspark_rf = RandomForestClassifier(
    labelCol=___, # this is how we set the target variable
    featuresCol=___)

# building the pipeline
class_pipe = Pipeline(stages = [___,___,___])
# an estimator is being used
class_pipe_model = class_pipe.fit(___)
# 'transforming' predictions onto the test data
pred_pipe_model = class_pipe_model.transform(___)
...
paramGrid = (ParamGridBuilder()
             .addGrid(pyspark_rf.maxDepth, [___])
             .addGrid(pyspark_rf.maxBins, [___])
#              .addGrid(pyspark_rf.numTrees, [5,10,15,20,25,30])
             .build())

# create how to evaluate the best model
rf_eval = MulticlassClassificationEvaluator(metricName=___)

# create the cross validator
rf_cv = CrossValidator(estimator=___, estimatorParamMaps=___, evaluator=___, numFolds=3, parallelism = 4)

# actually running the cross_validation and fitting the model to the training data
cv_rf_model = rf_cv.fit(___)

# testing how well the model ran by transforming the predictions onto the test data
cv_rf_pred = cv_rf_model.transform(___)
